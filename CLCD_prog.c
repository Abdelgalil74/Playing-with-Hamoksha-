...
{
	sint32  Local_s32Reminder,Local_s32Reverse=0,Local_s32NewN,Local_s32Length=0;
	sint8 Local_s8Counter;
	Local_s32NewN=Copys32Number;
	if(Copys32Number==0)
	{
		CLCD_voidSendData('0');
		return;
	}
	while(Local_s32NewN!=0)
	{
		Local_s32Length++;
		Local_s32NewN/=10;
	}
	...
	{
		Local_s32Reminder=Copys32Number%10;
		Local_s32Reverse=Local_s32Reverse*10+Local_s32Reminder;
		Copys32Number /= 10;
	}

	if(Local_s32Reverse<0)
	{
		CLCD_voidSendData('-');
		Local_s32Reverse*=-1;
	}

	for(Local_s8Counter=0;Local_s8Counter<Local_s32Length;Local_s8Counter++)
	{
		CLCD_voidSendData((Local_s32Reverse%10)+'0');
		Local_s32Reverse/=10;
	}

}
void CLCD_voidDisplayNumberPos(uint8 Copy_u8XPos,uint8 Copy_u8YPos,sint32 Copys32Number)
{
	CLCD_u8GoToXY(Copy_u8XPos,Copy_u8YPos);
	CLCD_voidDisplayNumber( Copys32Number);
}
